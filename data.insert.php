...
*/
get_header();
		$csv_filename = "http://localhost/globalrec/wp-content/themes/globalrec/insert/data.insert4-from-ods"; // name (no extension)
		...
		if ( $fp !== FALSE ) { // if the file exists and is readable
			...
			while ( ($fp_csv = fgetcsv($fp,$line_length,$delimiter,$enclosure)) !== FALSE ) { // begin main loop
				if ( $line == 0 ) {}
					...
					$members_type = $fp_csv[25]; // cf
					$members_occupation = $fp_csv[26]; // cf
					$organization_type = $fp_csv[27]; // cf
					$organization_scope = $fp_csv[28]; // cf
					$workplace_members = $fp_csv[29]; // cf
					...
					$objectives = $fp_csv[36]; // cf
					$education_training = $fp_csv[37]; // cf
					...
					$affiliations = $fp_csv[43]; // cf
					$funding = $fp_csv[44]; // cf
					$elections = $fp_csv[45]; // cf
					$member_benefits = $fp_csv[46]; // cf
					$credit_members = $fp_csv[47]; // cf
					$safety_technology = $fp_csv[48]; // cf
					$relationship_municipality = $fp_csv[49]; // cf
					$types_of_materials = $fp_csv[50]; // cf
					$middlemen = $fp_csv[51]; // cf
					$activities = $fp_csv[52]; // cf
					$sorting_spaces = $fp_csv[53]; // cf
					$treatment_organic_materials = $fp_csv[54]; // cf
					$challenges_access_waste = $fp_csv[55]; // cf
					...
					if ( $wpg != 0 ) {
						...
						//insert multiple anser option
						

						} // end if waste picker group has been inserted
						*/
